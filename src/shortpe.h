...
template<typename TConfig, typename TValidRegion, typename TSRStore,
		typename TStructuralVariantRecord>
inline void assembleSplitReads(TConfig const &c,
		TValidRegion const &validRegions, TSRStore const &srStore,
		std::vector<TStructuralVariantRecord> &svs) {
	...
	for (unsigned int file_c = 0; file_c < c.files.size(); ++file_c) {
		...
		idx[file_c] = sam_index_load(samfile[file_c],
				c.files[file_c].string().c_str());
	}
	...
	std::cout << '[' << boost::posix_time::to_simple_string(now) << "] "
			<< "Split-read assembly" << std::endl;
	boost::progress_display show_progress(2 * hdr->n_targets);

	faidx_t *fai = fai_load(c.genome.string().c_str());
	for (int32_t refIndex = 0; refIndex < hdr->n_targets; ++refIndex) {
		++show_progress;
		if (validRegions[refIndex].empty())
			continue;
		if (srStore[refIndex].empty())
			continue;

		// Load sequence
		int32_t seqlen = -1;
		std::string tname(hdr->target_name[refIndex]);
		char *seq = faidx_fetch_seq(fai, tname.c_str(), 0,
				hdr->target_len[refIndex], &seqlen);

		// Collect all split-read pos
		typedef boost::dynamic_bitset<> TBitSet;
		TBitSet hits(hdr->target_len[refIndex]);
		for (typename TPosReadSV::const_iterator it = srStore[refIndex].begin();
				it != srStore[refIndex].end(); ++it)
			hits[it->first.first] = 1;
		...
		for (unsigned int file_c = 0; file_c < c.files.size(); ++file_c) {
			// Read alignments
			for (typename TChrIntervals::const_iterator vRIt =
					validRegions[refIndex].begin();
					vRIt != validRegions[refIndex].end(); ++vRIt) {
				hts_itr_t *iter = sam_itr_queryi(idx[file_c], refIndex,
						vRIt->lower(), vRIt->upper());
				bam1_t *rec = bam_init1();
				while (sam_itr_next(samfile[file_c], iter, rec) >= 0) {
					if (rec->core.flag
							& (BAM_FQCFAIL | BAM_FDUP | BAM_FUNMAP
									| BAM_FSECONDARY | BAM_FSUPPLEMENTARY))
						continue;
					if ((rec->core.qual < c.minMapQual) || (rec->core.tid < 0))
						continue;
					if (!hits[rec->core.pos])
						continue;

					// Valid split-read
					std::size_t seed = hash_string(bam_get_qname(rec));
					typename TPosReadSV::const_iterator it =
							srStore[refIndex].find(
									std::make_pair(rec->core.pos, seed));
					if (it != srStore[refIndex].end()) {
						...
						if (svid == (int32_t) svs[svid].id) { // Should be always true
							...
							for (int i = 0; i < rec->core.l_qseq; ++i)
								sequence[i] = "=ACMGRSVTWYHKDBN"[bam_seqi(
										seqptr, i)];

							// Adjust orientation
							bool bpPoint = false;
							if (_translocation(svs[svid].svt)) {
								if (rec->core.tid == svs[svid].chr2)
									bpPoint = true;
							} else {
								// Only relevant for inversions
								if (svs[svid].svt == 0) {
									if (rec->core.pos + 25 > svs[svid].svStart)
										bpPoint = true;
									else
										bpPoint = false;
								} else if (svs[svid].svt == 1) {
									if (rec->core.pos + 25 > svs[svid].svEnd)
										bpPoint = true;
									else
										bpPoint = false;
								}
							}
							_adjustOrientation(sequence, bpPoint,
									svs[svid].svt);
							...
							if (seqStore[svid].size() < maxReadPerSV) {
								...
								if (_translocation(svs[svid].svt))
									insertSuccess = traStore[svid].insert(
											sequence).second;
								else
									insertSuccess = seqStore[svid].insert(
											sequence).second;
								// Store qualities
								if (insertSuccess) {
									if (_translocation(svs[svid].svt))
										traQualStore[svid].push_back(
												rec->core.qual);
									else
										qualStore[svid].push_back(
												rec->core.qual);
								}
							}
						}
		...
		for (uint32_t svid = 0; svid < seqStore.size(); ++svid) {
			...
			if (!msaSuccess) {
				...
				svs[svid].mapq = 0;
				for (uint32_t i = 0; i < qualStore[svid].size(); ++i)
					svs[svid].mapq += qualStore[svid][i];
				svs[svid].srSupport = seqStore[svid].size();
				svs[svid].srMapQuality = qualStore[svid][qualStore[svid].size()
						/ 2];
			}
		}
		// Clean-up
	...
	for (int32_t refIndex2 = 0; refIndex2 < hdr->n_targets; ++refIndex2) {
		++show_progress;
		if (validRegions[refIndex2].empty())
			continue;
		char *sndSeq = NULL;
		for (int32_t refIndex = refIndex2 + 1; refIndex < hdr->n_targets;
				++refIndex) {
			if (validRegions[refIndex].empty())
				continue;
			char *seq = NULL;
			...
			for (uint32_t svid = 0; svid < traStore.size(); ++svid) {
				...
				if (traStore[svid].size() > 1) {
					...
					if (seq == NULL) {
						int32_t seqlen = -1;
						std::string tname(hdr->target_name[refIndex]);
						seq = faidx_fetch_seq(fai, tname.c_str(), 0,
								hdr->target_len[refIndex], &seqlen);
					}
					if (sndSeq == NULL) {
						int32_t seqlen = -1;
						std::string tname(hdr->target_name[refIndex2]);
						sndSeq = faidx_fetch_seq(fai, tname.c_str(), 0,
								hdr->target_len[refIndex2], &seqlen);
					}
				...
				if (!msaSuccess) {
					svs[svid].consensus = "";
					//svs[svid].srSupport = 0;
					svs[svid].srAlignQuality = 1000;
					std::sort(traQualStore[svid].begin(),
											traQualStore[svid].end());
					svs[svid].mapq = 0;
					for (uint32_t i = 0; i < traQualStore[svid].size(); ++i)
						svs[svid].mapq += traQualStore[svid][i];
					svs[svid].srSupport = traStore[svid].size();
					svs[svid].srMapQuality =
							traQualStore[svid][traQualStore[svid].size() / 2];
				} else {
					// SR support and qualities
					std::sort(traQualStore[svid].begin(),
							traQualStore[svid].end());
...
#ifdef DEBUG
	std::cerr << "SVs at assembly" << std::endl;
...
template<typename TConfig, typename TValidRegion, typename TSRStore,
		typename TSampleLib>
inline void scanPEandSR(TConfig const &c, TValidRegion const &validRegions,
		std::vector<StructuralVariantRecord> &svs,
		std::vector<StructuralVariantRecord> &srSVs, TSRStore &srStore,
		TSampleLib &sampleLib) {
	typedef typename TValidRegion::value_type TChrIntervals;
	...
	boost::posix_time::ptime now =
			boost::posix_time::second_clock::local_time();
	std::cout << '[' << boost::posix_time::to_simple_string(now) << "] "
			<< "Paired-end and split-read scanning" << std::endl;
...
#pragma omp parallel for default(shared)
	for (unsigned int file_c = 0; file_c < c.files.size(); ++file_c) {
		...
		for (int32_t refIndex = 0; refIndex < (int32_t) hdr->n_targets;
				++refIndex) {
			++show_progress;
				...
				while (sam_itr_next(samfile[file_c], iter, rec) >= 0) {
					...
					for (std::size_t i = 0; i < rec->core.n_cigar; ++i) {
						if ((bam_cigar_op(cigar[i]) == BAM_CMATCH)
								|| (bam_cigar_op(cigar[i]) == BAM_CEQUAL)
								|| (bam_cigar_op(cigar[i]) == BAM_CDIFF)) {
							sp += bam_cigar_oplen(cigar[i]);
							rp += bam_cigar_oplen(cigar[i]);
						} else if (bam_cigar_op(cigar[i]) == BAM_CDEL) {
							if (bam_cigar_oplen(cigar[i]) > c.minRefSep)
								_insertJunction(readBp, seed, rec, rp, sp,
										false);
							rp += bam_cigar_oplen(cigar[i]);
							if (bam_cigar_oplen(cigar[i]) > c.minRefSep)
								_insertJunction(readBp, seed, rec, rp, sp,
							...
							if (bam_cigar_oplen(cigar[i]) > c.minClip)
								_insertJunction(readBp, seed, rec, rp, finalsp,
										scleft);
						} else if (bam_cigar_op(cigar[i]) == BAM_CREF_SKIP) {
							rp += bam_cigar_oplen(cigar[i]);
						} else {
							std::cerr << "Warning: Unknown Cigar operation!"
									<< std::endl;
						}
					}
					...
					if (rec->core.flag & BAM_FPAIRED) {
						// Single-end library
						if (sampleLib[file_c].median == 0)
							continue; // Single-end library
						...
						if (rec->core.flag
							...
							continue;
						if ((rec->core.mtid < 0)
								|| (rec->core.flag & BAM_FMUNMAP))
							continue;
						if (validRegions[rec->core.mtid].empty())
							continue;
						if ((_translocation(rec))
								&& (rec->core.qual < c.minTraQual))
							continue;

						// SV type
						int32_t svt = _isizeMappingPos(rec,
								sampleLib[file_c].maxISizeCutoff);
						if (svt == -1)
							continue;
						if ((c.svtcmd)
								&& (c.svtset.find(svt) == c.svtset.end()))
							continue;

						// Check library-specific insert size for deletions
						if ((svt == 2)
								&& (sampleLib[file_c].maxISizeCutoff
										> std::abs(rec->core.isize)))
							continue;
						...
						if (_firstPairObs(rec, lastAlignedPosReads)) {
							...
							if (_translocation(svt))
								matetra[file_c][hv] = std::make_pair(
										(uint8_t) rec->core.qual,
										alignmentLength(rec));
							else
								mateMap[hv] = std::make_pair(
										(uint8_t) rec->core.qual,
										alignmentLength(rec));
						} else {
							// Second read
							...
							if (_translocation(svt)) {
								// Inter-chromosomal
								if ((matetra[file_c].find(hv)
										== matetra[file_c].end())
										|| (!matetra[file_c][hv].first))
									continue; // Mate discarded
								TQualLen p = matetra[file_c][hv];
								pairQuality = std::min((uint8_t) p.first,
										(uint8_t) rec->core.qual);
								alenmate = p.second;
								matetra[file_c][hv].first = 0;
							} else {
								// Intra-chromosomal
								if ((mateMap.find(hv) == mateMap.end())
										|| (!mateMap[hv].first))
									continue; // Mate discarded
...
#pragma omp critical
							{
								bamRecord[svt].push_back(
										BamAlignRecord(rec, pairQuality,
												alignmentLength(rec), alenmate,
												sampleLib[file_c].median,
												sampleLib[file_c].mad,
												sampleLib[file_c].maxNormalISize));
							}
		...
		for (typename TReadBp::iterator it = readBp.begin(); it != readBp.end();
				++it) {
			std::sort(it->second.begin(), it->second.end(),
					SortJunction<Junction>());
		}

		// Collect split-read SVs
#pragma omp critical
		{
			if ((!c.svtcmd) || (c.svtset.find(2) != c.svtset.end()))
				selectDeletions(c, readBp, srBR);
			if ((!c.svtcmd) || (c.svtset.find(3) != c.svtset.end()))
				selectDuplications(c, readBp, srBR);
			if ((!c.svtcmd) || (c.svtset.find(0) != c.svtset.end())
					|| (c.svtset.find(1) != c.svtset.end()))
				selectInversions(c, readBp, srBR);
			if ((!c.svtcmd) || (c.svtset.find(4) != c.svtset.end()))
				selectInsertions(c, readBp, srBR);
			if ((!c.svtcmd)
					|| (c.svtset.find(DELLY_SVT_TRANS) != c.svtset.end())
					|| (c.svtset.find(DELLY_SVT_TRANS + 1) != c.svtset.end())
					|| (c.svtset.find(DELLY_SVT_TRANS + 2) != c.svtset.end())
					|| (c.svtset.find(DELLY_SVT_TRANS + 3) != c.svtset.end()))
	...
	now = boost::posix_time::second_clock::local_time();
	std::cout << '[' << boost::posix_time::to_simple_string(now) << "] "
			<< "Paired-end clustering" << std::endl;
	...
	for (uint32_t svt = 0; svt < srBR.size(); ++svt) {
		for (uint32_t i = 0; i < srBR[svt].size(); ++i) {
			...
			if ((srBR[svt][i].svid != -1) && (srBR[svt][i].rstart != -1)) {
				if (srBR[svt][i].rstart
						< (int32_t) hdr->target_len[srBR[svt][i].chr])
					srStore[srBR[svt][i].chr].insert(
							std::make_pair(
									std::make_pair(srBR[svt][i].rstart,
											srBR[svt][i].id),
									srBR[svt][i].svid));
				if (srBR[svt][i].chr != srBR[svt][i].chr2) {
					...
					if (srBR[svt][i].rstart
							< (int32_t) hdr->target_len[srBR[svt][i].chr2])
						srStore[srBR[svt][i].chr2].insert(
								std::make_pair(
										std::make_pair(srBR[svt][i].rstart,
												srBR[svt][i].id),
										srBR[svt][i].svid));
...
inline void mergeSort(std::vector<StructuralVariantRecord> &pe,
		std::vector<StructuralVariantRecord> &sr) {
	...
	for (int32_t svt = 0; svt < 10; ++svt) {
		for (int32_t i = 0; i < (int32_t) sr.size(); ++i) {
			if (sr[i].svt != svt)
				continue;
			if ((sr[i].srSupport == 0) || (sr[i].srAlignQuality == 0)){
#ifdef DEBUG
				outputSingleSV(sr[i] );
#endif
			...
			bool svExists = false;
			typename TVariants::iterator itOther = std::lower_bound(pe.begin(),
					pe.end(),
					StructuralVariantRecord(sr[i].chr,
							std::max(0, sr[i].svStart - searchWindow),
							sr[i].svEnd), SortSVs<StructuralVariantRecord>());
			for (;
					((itOther != pe.end())
							&& (std::abs(itOther->svStart - sr[i].svStart)
									< searchWindow)); ++itOther) {
				if ((itOther->svt != svt) || (itOther->precise))
					continue;
				if ((sr[i].chr != itOther->chr)
						|| (sr[i].chr2 != itOther->chr2))
					continue;  // Mismatching chr

				// Breakpoints within PE confidence interval?
				if ((itOther->svStart + itOther->ciposlow < sr[i].svStart)
						&& (sr[i].svStart
								< itOther->svStart + itOther->ciposhigh)) {
					if ((itOther->svEnd + itOther->ciendlow < sr[i].svEnd)
							&& (sr[i].svEnd
									< itOther->svEnd + itOther->ciendhigh)) {
						svExists = true;
						// Augment PE record
			...
			if (!svExists) {
				...
				for (int32_t j = i + 1; j < (int32_t) sr.size(); ++j) {
					if (std::abs(sr[i].svStart - sr[j].svStart)
							> precSearchWindow)
						break;
					if (sr[i].svt != sr[j].svt)
						continue;   // Mismatching SV types
					if ((sr[i].chr != sr[j].chr) || (sr[i].chr2 != sr[j].chr2))
						continue;  // Mismatching chr

					// Breakpoints within PE confidence interval?
					if ((sr[j].svStart + sr[j].ciposlow <= sr[i].svStart)
							&& (sr[i].svStart <= sr[j].svStart + sr[j].ciposhigh)) {
						if ((sr[j].svEnd + sr[j].ciendlow <= sr[i].svEnd)
								&& (sr[i].svEnd <= sr[j].svEnd + sr[j].ciendhigh)) {
							// Duplicate, keep better call
							if ((sr[i].srSupport < sr[j].srSupport)
									|| ((i < j)
											&& (sr[i].srSupport
													== sr[j].srSupport)))
								preciseDuplicate = true;
						}
					}
				}
				...
				if (!preciseDuplicate) {
					...
					sort(pe.begin(), pe.end(),
							SortSVs<StructuralVariantRecord>());
				}
			}
		}
	}
